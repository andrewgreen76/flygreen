...
if (INGAME) {i2c_write(background(x,y)|SpriteWrite(x,y,Sprite)|DotsWrite(x,y,Sprite)|LiveWrite(x,y)|FruitWrite(x,y));}else{
i2c_write(0xff-(background(x,y)|SpriteWrite(x,y,Sprite)));
}}else if (render0_picture1==1){
i2c_write(pgm_read_byte(&back[x+(y*128)]));}}
i2c_stop();
}}
