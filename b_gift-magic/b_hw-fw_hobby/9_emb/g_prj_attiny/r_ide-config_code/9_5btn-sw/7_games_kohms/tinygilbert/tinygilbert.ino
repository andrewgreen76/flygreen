...
#define FPS_Control while((currentMillis-MemMillis)<25){currentMillis=millis();}MemMillis=currentMillis
...
uint16_t WSPRITE=(pgm_read_byte(&SPRITES[0]));
uint16_t HSPRITE=(pgm_read_byte(&SPRITES[1]));
...
for (x = 0; x < 128; x++){ 
i2c_write(SPEED_BLITZ(32,4,x,y,0,start));
}
i2c_stop();
}}
...
for (x=0;x<LIVE-1;x++){
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite26[t]));}
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite27[t]));}
}
for (x=0;x<11+(3-(LIVE-1));x++){
for(t=0;t<8;t++){i2c_write(0x00);}}
if ((pgm_read_byte(&KeyinLevel[levelType])==keyS)&&(timer<=30)) {
for (x=0;x<4;x++){
i2c_write(pgm_read_byte(&sprite12[x]));}}else{
for (x=0;x<4;x++){
for(t=0;t<4;t++){i2c_write(0x00);}
}}  
for (m = 1; m < 8; m++)
{
...
while1=1;
Start=4-step4; 
#define decalIN for(decal=0;decal<DSprite->x4decalage;decal++){i2c_write(0X00);PrecessQuit}// for main sprite scr pix2pix
#define decalOUT for(decal=0;decal<4-DSprite->x4decalage;decal++){i2c_write(0X00);PrecessQuit}// for main sprite scr pix2pix
while(while1){
if ((Map[m][n]==7)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite7[t]));PrecessQuit}Start=0;
// main sprite 
...
if (LorR==1){
if (MainAnim==0) {
for(t=0;t<4;t++){ i2c_write(pgm_read_byte(&sprite20[t])*VSlideOut);PrecessQuit}Start=0;
}
if (MainAnim==1) { 
for(t=0;t<4;t++){ i2c_write(pgm_read_byte(&sprite22[t])*VSlideOut);PrecessQuit}Start=0;
}
if (MainAnim==2) {
for(t=0;t<4;t++){ i2c_write(pgm_read_byte(&sprite24[t])*VSlideOut);PrecessQuit}Start=0; 
} 
}else{  
if (MainAnim==0) {
for(t=0;t<4;t++){ i2c_write(pgm_read_byte(&sprite26[t])*VSlideOut);PrecessQuit}Start=0;   
}
if (MainAnim==1) {
for(t=0;t<4;t++){ i2c_write(pgm_read_byte(&sprite28[t])*VSlideOut);PrecessQuit}Start=0;   
}
if (MainAnim==2) {
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite30[t])*VSlideOut);PrecessQuit}Start=0;  
}}
...
if (LorR==1){
if (MainAnim==0) {
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite21[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}
if (MainAnim==1) {
for(t=0;t<4;t++){ i2c_write(pgm_read_byte(&sprite23[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}
if (MainAnim==2) {
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite25[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
} }else{  
if (MainAnim==0) {
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite27[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}
if (MainAnim==1) {
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite29[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;} 
}
if (MainAnim==2) {
for(t=0;t<4;t++){i2c_write(pgm_read_byte(&sprite31[t])*VSlideOut);PrecessQuit Start=DSprite->x4decalage;}
}}
//fin main sprite
}else if ((Map[m][n]==1)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite1[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==2)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite2[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==3)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite3[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==4)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite4[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==8)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite8[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==15)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite15[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==16)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite16[t]));PrecessQuit}Start=0;
}
else if ((Map[m][n]==11)&&(while1!=0)) {
if (timer>30){
for(t=Start;t<4;t++){
i2c_write(pgm_read_byte(&sprite11[t]));PrecessQuit}Start=0;
}else{
for(t=Start;t<4;t++){
i2c_write(pgm_read_byte(&sprite12[t]));PrecessQuit}Start=0;
}}else if ((Map[m][n]==13)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite13[t]));PrecessQuit}Start=0;
}else if ((Map[m][n]==14)&&(while1!=0)) {
for(t=Start;t<4;t++){i2c_write(pgm_read_byte(&sprite14[t]));PrecessQuit}Start=0;
}
else{if (while1!=0){
for(t=Start;t<4;t++){
i2c_write(0x00);PrecessQuit}Start=0;
}}
if (while1!=0) {n++; }
}
i2c_stop();
